...
# active_poke = 'Left_poke2022-02-22T12_42_05'
# ZTP = []
...
def FED3_both_active_pokes(import_location, export_location, poke_to_analyse, active_poke, ZTP, setup, TRANGE, BASELINE_PER, ARTIFACT, save_to_csv_zScore, save_to_csv_dFF, save_to_csv_F415, save_to_csv_F475):
    ...
    **Importing the Data**
    ...
    if data_type == 'NPM':
        ...
        LEFT   = [event for event in list_events if 
                  ('Left' in event or 'left' in event)][0]
        RIGHT  = [event for event in list_events if 
                  ('Right' in event or 'right' in event)][0]
        ...
        **Setup the variables for the data you want to extract**
        ...
        LEFT   = list(set(possible_lefts).intersection(data_original.epocs.keys()))
        RIGHT  = list(set(possible_rights).intersection(data_original.epocs.keys()))
        ...
        if len(PELLET) != 1 or len(LEFT) != 1 or len(RIGHT) != 1:
            print('More possible pellet retrieval or active poke names are needed in this code.')
            sys.exit()
        PELLET, LEFT, RIGHT = PELLET[0], LEFT[0], RIGHT[0]
        
    # Create a new event that is a combination of the left and right pokes.
    LEFT_RIGHT = 'Poke'
    onset_def  = np.sort(np.append(data_original.epocs[LEFT].onset, data_original.epocs[RIGHT].onset))
    offset_def = np.sort(np.append(data_original.epocs[LEFT].onset, data_original.epocs[RIGHT].onset))
    offset_def = np.append(offset_def[1:], np.inf)
    SCORE_DICT = {"name":     LEFT_RIGHT,
                  "onset":    onset_def,
                  "offset":   offset_def,
                  "data":     np.sort(np.append(data_original.epocs[LEFT].data, data_original.epocs[RIGHT].data))}
    data_original.epocs[LEFT_RIGHT] = StructType(SCORE_DICT)
    ...
    **Use epoc_filter to extract data around our epoc event**
    ...
    data = epoc_filter(data_original, LEFT_RIGHT, t=TRANGE, values=ZTP) 
    ...
    **Make a bar plot**
    ...
    # Create lists of the start times of the following events.
    LEFT_ON       = data.epocs[LEFT].onset # Left nose pokes
    RIGHT_ON      = data.epocs[RIGHT].onset # Right nose pokes
    LEFT_RIGHT_ON = data.epocs[LEFT_RIGHT].onset # Left and right nose pokes
    ...
    if len(LEFT_RIGHT_ON) != len(zall):
        print('\nPLEASE NOTE: the last '+str(len(LEFT_RIGHT_ON)-len(zall)) + ' events '+
              'have been excluded, because their window durations go past the end of '+
              'the recording.\n')
        excluded_nums = LEFT_RIGHT_ON[len(zall):]
        LEFT_RIGHT_ON = LEFT_RIGHT_ON[:len(zall)]
        LEFT_ON  = [onset for onset in LEFT_ON  if (onset not in excluded_nums)]
        RIGHT_ON = [onset for onset in RIGHT_ON if (onset not in excluded_nums)]
    ...
    for i in range(len(LEFT_ON)):
        all_events.append({'Time point':LEFT_ON[i], 'Event':'Left nose poke', 'Poke number':i+1, 'Rewarded':is_rewarded(active_poke, 'Left')})
    for i in range(len(RIGHT_ON)):
        all_events.append({'Time point':RIGHT_ON[i], 'Event':'Right nose poke', 'Poke number':i+1, 'Rewarded':is_rewarded(active_poke, 'Right')})
    ...
    if active_poke == 'Changing':
        ...
        for i in range(len(all_events)):
            if 'nose poke' in all_events[i]['Event']:
    ...
    pokes        = [(dict1['Event'], dict1['Poke number']) for dict1 in all_events if 'nose poke' in dict1['Event']]
    rewarded     = [dict1['Rewarded'] for dict1 in all_events if 'nose poke' in dict1['Event']]
    reward_poke  = dict(zip(pokes, rewarded))
    ...
    for stat in results.keys():
        ...
        headers = [reward_poke[key]+' '+key[0]+' '+str(key[1]) for key in reward_poke.keys()]
        ...
        ind_headers = [[reward_poke[poke], poke[0], poke[1]] for poke in reward_poke.keys()]
        ind_headers = pd.DataFrame(dict(zip(headers, ind_headers)),index=['Rewarded','Left/right poke','Poke number'])
        ...
        results[stat] = results[stat].sort_values(by=['Rewarded','Left/right poke','Poke number'], axis=1, ascending=[False,True,True])
        results[stat] = results[stat].drop(['Rewarded','Left/right poke','Poke number'])
        ...
        ind_headers = ind_headers.sort_values(by=['Rewarded','Left/right poke','Poke number'], axis=1, ascending=[False,True,True])
    ...
    filename_zScore = os.path.join (
        ...
        (os.path.basename(filename) + "_zScore" + "_analyse" + "_left_and_right_" + 
         "active_is_" + active_poke.lower() + '_' + setup.replace(' ','_') + ".csv"))
    ...
    filename_dFF = os.path.join (
        # os.path.dirname (filename), # saves file in same directory
        export_location,  # change the path to where you want files saved
        (os.path.basename(filename) + "_dFF" + "_analyse" + "_left_and_right_" + 
         "active_is_" + active_poke.lower() + '_' + setup.replace(' ','_') + ".csv"))
    ...
    filename_F415 = os.path.join (
        # os.path.dirname (filename), # saves file in same directory
        export_location,  # change the path to where you want files saved
        (os.path.basename(filename) + ISOS + "_analyse" + "_left_and_right_" + 
         "active_is_" + active_poke.lower() + '_' + setup.replace(' ','_') + ".csv"))
        # os.path.basename(filename) + "_DFF" + str(SHOCK_CODE) + ".csv")
    ...
    filename_F475 = os.path.join (
        # os.path.dirname (filename), # saves file in same directory
        export_location,  # change the path to where you want files saved
        (os.path.basename(filename) + GCaMP + "_analyse" + "_left_and_right_" + 
         "active_is_" + active_poke.lower() + '_' + setup.replace(' ','_') + ".csv"))
        # os.path.basename(filename) + "_DFF" + str(SHOCK_CODE) + ".csv")
    ...
    #     #os.path.dirname (filename), # saves file in same directory
    #     export_location, # change the path to where you want files saved
    #     os.path.basename(filename) + "_AUC" + "_both_active_pokes" + ".csv")
