...
def FED3_pellet_retrievals(import_location, export_location, poke_to_analyse, active_poke, ZTP, setup, TRANGE, BASELINE_PER, ARTIFACT, save_to_csv_zScore, save_to_csv_dFF, save_to_csv_F415, save_to_csv_F475):
    ...
    **Importing the Data**
    ...
    if data_type == 'NPM':
        ...
        list_events = list(data.epocs.keys())
        ...
        **Setup the variables for the data you want to extract**
        ...
        PELLET = list(set(possible_pellets).intersection(data.epocs.keys()))
        if len(PELLET) == 0:
    ...
    **Use epoc_filter to extract data around our epoc event**
    ...
    data = epoc_filter(data, PELLET, t=TRANGE, values=ZTP) 
    ...
    **Make a bar plot**
    ...
    pellet_onsets = data.epocs[PELLET].onset
    if len(pellet_onsets) != len(zall):
        print('\nPLEASE NOTE: the last '+str(len(pellet_onsets)-len(zall)) + ' events '+
    ...
    for stat in results.keys():
        ...
        # Create a pandas dataframe for the pellet retrieval numbers and Z-score data.
        headers = ['Pellet '+str(i) for i in range(1,len(results[stat])+1)]
        ...
        results[stat].insert(0, 'Mean of pellets', mean_column)
    ...
    filename_zScore = os.path.join (
        ...
        os.path.basename(filename) + "_zScore" + "_pellet_retrievals_" + setup.replace(' ','_') + ".csv")
    ...
    filename_dFF = os.path.join (
        # os.path.dirname (filename), # saves file in same directory
        export_location,  # change the path to where you want files saved
        os.path.basename(filename) + "_dFF" + "_pellet_retrievals_" + setup.replace(' ','_') + ".csv")
    ...
    filename_F415 = os.path.join (
        # os.path.dirname (filename), # saves file in same directory
        export_location,  # change the path to where you want files saved
        os.path.basename(filename) + ISOS + "_pellet_retrievals_" + setup.replace(' ','_') + ".csv")
        # os.path.basename(filename) + "_DFF" + str(SHOCK_CODE) + ".csv")
    ...
    filename_F475 = os.path.join (
        # os.path.dirname (filename), # saves file in same directory
        export_location,  # change the path to where you want files saved
        os.path.basename(filename) + GCaMP + "_pellet_retrievals_" + setup.replace(' ','_') + ".csv")
        # os.path.basename(filename) + "_DFF" + str(SHOCK_CODE) + ".csv")
    ...
    #     #os.path.dirname (filename), # saves file in same directory
    #     export_location, # change the path to where you want files saved
    #     os.path.basename(filename) + "_AUC" + "_pellet_retrievals" + ".csv")
